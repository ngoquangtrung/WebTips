...
function sendreply(idrepcmt,nameuser,idtextarea){
	let content=document.getElementById("text_"+idtextarea).value;
	let idpost= document.getElementById("idpostcmt").value;
	if(content==null||content==""){
		alert("Bạn chưa nhập nội dung phản hồi")
		return false;
...
function deleteCmtbyAdmin(idcmt){
...
function like_unlikecmt(idcmt){
	...
	$.ajax({
						...
						if(rs!="login"){
							...
							if(rs=="Bỏ thích"){
								document.getElementById("like"+idcmt).style.color='blue';
								document.getElementById("like"+idcmt).style.fontWeight='500';
							}else{
								document.getElementById("like"+idcmt).style.color='black';
								document.getElementById("like"+idcmt).style.fontWeight='300';
							}
